...
public:
    ...
    void setOrderWithNumberOfLoudspeakers(const long _nbLoudSpeakers);
    void setMode(const Modes _mode, bool _doSetupIO = true);
    void setVectorSize(const long _vectorSize);
    void setSampleRate(const long _samplingRate);
    
    
    virtual void IOChangeCallback() {};
    virtual void DSPChangeCallback() {};
    
    
    // getters :
    inline long  getOrder() {return m_order;}
    inline long  getVectorSize() {return m_vectorSize;}
    inline long  getSampleRate() {return m_sampleRate;}
