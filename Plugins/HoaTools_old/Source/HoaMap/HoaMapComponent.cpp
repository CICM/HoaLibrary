...
{    
    ...
    if(m_source_selected != -1)
    {
        ...
        if (m_drag_mode == 1)
        {
            m_processor->sourceSetRadius(m_source_selected, mouse.getDistanceFrom(Point<float>(0., 0.)) / m_processor->getZoom());
        }
        else if(m_drag_mode == 2)
        {
            m_processor->sourceSetAzimuth(m_source_selected, mouse.getAngleToPoint(Point<float>(0., 0.)));
        }
        else if(m_drag_mode == 3)
        {
            m_processor->sourceSetAbscissa(m_source_selected, mouse.getX() / m_processor->getZoom());
        }
        else if(m_drag_mode == 4)
        {
            m_processor->sourceSetOrdinate(m_source_selected, mouse.getY() / m_processor->getZoom());
        }
        else
        {
            m_processor->sourceSetCartesian(m_source_selected, mouse.getX() / m_processor->getZoom(), mouse.getY() / m_processor->getZoom());
        }
