...
typedef struct State { 
	...
	double samplerate;
	double m_grainsize_3;
	double m_feedback_2;
	double m_delay_5;
	double m_rarefaction_4;
	int vectorsize;
	int __exception;
	...
	inline int perform(t_sample ** __ins, t_sample ** __outs, int __n) { 
		...
		double mul_99 = (1 * m_feedback_2);
		double clamp_108 = ((mul_99 <= 0) ? 0 : ((mul_99 >= 1) ? 1 : mul_99));
		double mul_103 = (1 * m_delay_5);
		double mul_114 = (mul_103 * 0.5);
		double mul_101 = (1 * m_grainsize_3);
		double rdiv_111 = safediv(1000, mul_101);
		samples_to_seconds = (1 / samplerate);
		double mul_105 = (1 * m_rarefaction_4);
	...
	inline void set_feedback(double __value) {
		m_feedback_2 = (__value < 0 ? 0 : (__value > 0.99 ? 0.99 : __value));
	};
	inline void set_grainsize(double __value) {
		m_grainsize_3 = (__value < 1 ? 1 : (__value > 5000 ? 5000 : __value));
	};
	inline void set_rarefaction(double __value) {
		m_rarefaction_4 = (__value < 0 ? 0 : (__value > 1 ? 1 : __value));
	};
	inline void set_delay(double __value) {
		m_delay_5 = (__value < 0 ? 0 : (__value > 5000 ? 5000 : __value));
	};
...
void setparameter(CommonState *cself, long index, double value, void *ref) {
	State * self = (State *)cself;
	switch (index) {
		case 0: self->set_feedback(value); break;
		case 1: self->set_grainsize(value); break;
		case 2: self->set_rarefaction(value); break;
		case 3: self->set_delay(value); break;
...
void getparameter(CommonState *cself, long index, double *value) {
	State *self = (State *)cself;
	switch (index) {
		case 0: *value = self->m_feedback_2; break;
		case 1: *value = self->m_grainsize_3; break;
		case 2: *value = self->m_rarefaction_4; break;
		case 3: *value = self->m_delay_5; break;
...
void * create(double sr, long vs) {
	...
	// initialize parameter 0 ("m_feedback_2")
	pi = self->__commonstate.params + 0;
	pi->name = "feedback";
	pi->paramtype = GENLIB_PARAMTYPE_FLOAT;
	pi->defaultvalue = self->m_feedback_2;
	pi->defaultref = 0;
	...
	// initialize parameter 1 ("m_grainsize_3")
	pi = self->__commonstate.params + 1;
	pi->name = "grainsize";
	pi->paramtype = GENLIB_PARAMTYPE_FLOAT;
	pi->defaultvalue = self->m_grainsize_3;
	pi->defaultref = 0;
	pi->hasinputminmax = false;
	pi->inputmin = 0; 
	pi->inputmax = 1; 
	...
	pi->exp = 0;
	pi->units = "";		// no units defined
	// initialize parameter 2 ("m_rarefaction_4")
	pi = self->__commonstate.params + 2;
	pi->name = "rarefaction";
	pi->paramtype = GENLIB_PARAMTYPE_FLOAT;
	pi->defaultvalue = self->m_rarefaction_4;
	pi->defaultref = 0;
	pi->hasinputminmax = false;
	pi->inputmin = 0; 
	pi->inputmax = 1; 
	...
	pi->exp = 0;
	pi->units = "";		// no units defined
	// initialize parameter 3 ("m_delay_5")
