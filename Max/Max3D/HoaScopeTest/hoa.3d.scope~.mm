...
void hoa_scope_paint(t_hoa_scope* x, t_object *patcherview);
...
void hoa_scope_paint(t_hoa_scope* x, t_object *patcherview)
{
    ...
    object_method(patcherview, gensym("nativewindow"), (void **)&cocoa_view);
    x->j_component->addToDesktop(ComponentPeer::StyleFlags::windowIsTemporary, cocoa_view);
    patcherview_get_rect(patcherview, &rect);
    //post("rect %f %f %f %f", (float)rect.x, (float)rect.y, (float)rect.width, (float)rect.height);
