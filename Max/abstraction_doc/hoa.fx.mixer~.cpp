
// ------------------------------------------ hoa.fx.mixer~ ------------------------------------------ //

/**
 @file      hoa.fx.mixer~.cpp
 @name      hoa.fx.mixer~
 @realname  hoa.fx.mixer~
 @type      abstraction
 @module    hoa
 @author    Julien Colafrancesco, Pierre Guillot, Eliott Paris.
 
 @digest
 An ambisonic harmonic mixer.
 
 @description
 <o>hoa.fx.mixer~</o> helps to understand ambisonics and the behavior of the circular or spherical harmonics.
 This patcher requires to be loaded by a <o>hoa.process~</o> object to work.
 
 @discussion
 <o>hoa.fx.mixer~</o> helps to understand ambisonics and the behavior of the circular or spherical harmonics.
 This patcher requires to be loaded by a <o>hoa.process~</o> object to work.
 
 @category ambisonics, hoa objects, audio, MSP, abstraction
 
 @seealso hoa.process~, hoa.fx.decorrelation~, hoa.fx.delay~, hoa.fx.gain~, hoa.fx.grain~, hoa.fx.granular~, hoa.fx.mirror~, hoa.fx.ringmod~
 */