...
void *plug_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	if(atom_gettype(argv+1) == A_SYM)
		...
		patchname = gensym("hoa.all");

	test = newobject_sprintf(x, "@maxclass newobj @text %s", patchname->s_name);
	if(jbox_get_maxclass(test) != gensym("jpatcher"))
		return NULL;
	else
		object_free(test);
...
{
    int harm, virtualSpeak;
    
    virtualSpeak = floor((float)index / (float)ninlet);
	...
	if (mode == 1) 
		...
		return (t_object *)newobject_sprintf(patcher, "@maxclass inlet @comment \"Virtual Microphone %i Input %i\" @patching_rect %i 0 20 20 ", virtualSpeak, index, harm * 100 + (index * 100) / ninlet);
}

t_object *plug_outlet(t_object *patcher, int index, int order, int noutlet, int mode)
{
	int harm, last, virtualSpeak;
    
    virtualSpeak = floor((float)index / (float)noutlet);
	...
	if (mode == 2)
		...
		return newobject_sprintf(patcher, "@maxclass outlet @comment \"Virtual Microphone %i Output %i\" @patching_rect %i 600 20 20 ", virtualSpeak, index, harm * 100 + (index * 100) / noutlet);
	else
...
t_object *plug_script(t_object *patcher, int index, int order, int ninlet, int noutlet, int mode)
{
	...
	index %= ninlet;
	if (mode == 2)
		return newobject_sprintf(patcher, "@maxclass newobj @text \"hoa.plug_script %i %i %i %i %i\" @patching_rect %i 300 20 20",  last, order, mode, ninlet, noutlet, harm * 100 + (index * 100) / ninlet);
    if (mode == 3)
		return newobject_sprintf(patcher, "@maxclass newobj @text \"hoa.plug_script %i %i %i %i %i\" @patching_rect %i 300 20 20",  last, order, mode, ninlet, noutlet, harm * 100 + (index * 100) / ninlet);
	else
		return newobject_sprintf(patcher, "@maxclass newobj @text \"hoa.plug_script %i %i %i %i %i\" @patching_rect %i 300 20 20", plug_harmonic(harm, order), order, mode, ninlet, noutlet, harm * 100 + (index * 100) / ninlet);
...
t_object *plug_patch(t_object *patcher, t_symbol *s, int index, int order, int mode, int noutlet, t_atom *argv, int argc)
{
	...
	if (mode == 2)
        ...
        return newobject_sprintf(patcher, "@maxclass newobj @text \"%s %i %i %s\" @patching_rect %i 500 20 20", s->s_name, virtualSpeak, order, argus, index * 100);
	else
...
void plug_router(t_object *x, int order, t_symbol *s, int mode, t_atom *argv, int argc)
{
	...
	if(mode == 1)
	{
		...
		{
			patchs[i] = plug_patch(x, s, i, order, mode, 0, argv, argc);
    ...
    else
	{
		...
		for(i = 0; i < order; i++)
		{
			patchs[i] = plug_patch(x, s, i, order, mode, noutlet, argv, argc);
			sprintf(name, "%s (%i)", s->s_name,  i);
			jpatcher_set_title(patchs[i], gensym(name));
			for(j = 0; j < ninlet; j++)
			{
		...
		outlets = (t_object **)getbytes(order * noutlet * sizeof(t_object *));
		for(i = 0; i < order * noutlet; i++)
		{
			outlets[i] = plug_outlet(x, i, order, noutlet, mode);
			plug_connect((t_object *)x, patchs[(int)(i /noutlet)], (int)(i % noutlet), (t_object *)outlets[i], 0);
		}
