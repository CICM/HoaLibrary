...
int C74_EXPORT main(void)
{
    ...
    // The target numbering/arguments depends on the current mode and the number of loded instances.
    // In <m>planewaves</m> mode (2d/3d), the message <b>target 1</b> will target the first instance corresponding to the first channel.
    // In 2d <m>harmonics</m> mode, target argument correspond to the harmonic index, the harmonics index go to <b>-order</b> to <b>order</b>.
    // In 3d <m>harmonics</m> mode, the <m>target</m> message takes two arguments. First one is the ambisonic band (between 0 and the <b>order</b>), the second one correspond to the harmonic index (from <b>-order</b> to <b>order</b>).
