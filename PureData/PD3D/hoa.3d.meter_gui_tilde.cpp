    ...
    if(argc && argv)
    {
        if(atom_gettype(argv) == A_FLOAT)
        {
            ...
            if(d != x->f_meter->getNumberOfChannels())
            {
                ...
                av = new t_atom[x->f_meter->getNumberOfChannels() * 2];
				for(int i = 0, j = 0; i < x->f_meter->getNumberOfChannels() * 2; i+=2, j++)
				{
					atom_setfloat(av+i, x->f_meter->getChannelAzimuth(j) / HOA_2PI * 360.);
					atom_setfloat(av+i+1, x->f_meter->getChannelElevation(j) / HOA_2PI * 360.);
					x->f_over_leds[i] = 0;
				}	
                angles_set(x, NULL, x->f_meter->getNumberOfChannels() * 2, av);
                delete [] av;
