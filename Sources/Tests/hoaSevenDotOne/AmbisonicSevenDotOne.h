...
#ifndef DEF_AMBISONICSEVENDOTONE
#define DEF_AMBISONICSEVENDOTONE
...
public:
	AmbisonicSevenDotOne(long anOrder,  double aFrontDelta = 30., double aSurroundDelta = 110., double aBackSurroundDelta = 135., long aVectorSize = 0);
	...
	double	getFractionalOrderBackSurround();

	void	setVectorSize(int aVectorSize);
	~AmbisonicSevenDotOne();
	...
	{	
		for(int j = 0; j < m_number_of_harmonics; j++)
		{
			...
			gsl_vector_set(m_input_vector_back_surround, j, aInputs[j]);
		}
		...
		gsl_blas_dgemv(CblasTrans, 1.0, m_microphones_matrix_back_surround, m_input_vector_back_surround, 0.0, m_output_vector_back_surround);
		...
		aOutputs[6] = gsl_vector_get(m_output_vector_stereo, 1) * m_scale_factor_stereo;
		aOutputs[2] = gsl_vector_get(m_output_vector_surround, 0) * m_scale_factor_surround;
		aOutputs[5] = gsl_vector_get(m_output_vector_surround, 1) * m_scale_factor_surround;
		aOutputs[3] = gsl_vector_get(m_output_vector_surround, 0) * m_scale_factor_back_surround;
		aOutputs[4] = gsl_vector_get(m_output_vector_surround, 1) * m_scale_factor_back_surround;
		aOutputs[7] = 0.;
		for(int j = 0; j < 7; j++)
			aOutputs[7] += aOutputs[j];
		aOutputs[7] /= 7.;
		aOutputs[7] *= 0.017096;
		aOutputs[7] -= m_last_sample;
	...
	{	
		for(int i = 0; i < m_vector_size; i++)
		{
			for(int j = 0; j < m_number_of_harmonics; j++)
			{
				...
				gsl_vector_set(m_input_vector_back_surround, j, aInputs[j][i]);
			}
			...
			aOutputs[6][i] = gsl_vector_get(m_output_vector_stereo, 1) * m_scale_factor_stereo;
			aOutputs[2][i] = gsl_vector_get(m_output_vector_surround, 0) * m_scale_factor_surround;
			aOutputs[5][i] = gsl_vector_get(m_output_vector_surround, 1) * m_scale_factor_surround;
			aOutputs[3][i] = gsl_vector_get(m_output_vector_surround, 0) * m_scale_factor_back_surround;
			aOutputs[4][i] = gsl_vector_get(m_output_vector_surround, 1) * m_scale_factor_back_surround;
			aOutputs[7][i] = 0.;
			for(int j = 0; j < 7; j++)
				aOutputs[7][i] += aOutputs[j][i];
			aOutputs[7][i] /= 7.;
			aOutputs[7][i] *= 0.017096;
			aOutputs[7][i] -= m_last_sample;
			m_last_sample = -0.982904 * aOutputs[7][i];
		}
	}	
};
