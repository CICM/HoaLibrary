...
ZeroLatencyConvolver::ZeroLatencyConvolver(long aMinimumSize, long aMaximumSize) 
{
	...
	{
		m_fft.push_back(new FftConvolution(windowSize, 1));
	}
...
void ZeroLatencyConvolver::setImpulseResponse(float* anImpulseResponse, long aSize)
{
    ...
    {
        Cicm_Signal *datas  = new Cicm_Signal[m_minimum_size];
    ...
    for(int i = 0; i < m_number_of_ffts - 1; i++)
	{
        ...
        {
            Cicm_Signal *datas  = new Cicm_Signal[windowSize];
    ...
    if(aSize >= offset)
    {
        ...
        Cicm_Signal *datas  = new Cicm_Signal[dataSize];
        if(numberOfFFT != m_fft[m_number_of_ffts-1]->getNumberOfInstances())
        {
            m_fft.pop_back();
            m_fft.push_back(new FftConvolution(m_maximum_size, numberOfFFT));
        }
