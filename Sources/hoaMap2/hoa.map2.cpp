...
typedef struct  _hoamap 
{
	...
	void*		f_outCarte;
	void*		f_out;
	//vector <SourcePoint*>	f_sourcePoints;
	SourceManager*	map;
} t_hoamap;
...
void hoamap_output_number_of_sources(t_hoamap *x);
...
int main()
{
	...
	c = class_new("hoa.map2", (method)hoamap_new, (method)hoamap_free, (short)sizeof(t_hoamap), 0L, A_GIMME, 0);
...
void hoamap_free(t_hoamap *x)
{
	//delete x->f_sourcePoints;
	//x->f_sourcePoints.clear();
	delete x->map;
	jbox_free(&x->j_box);
}
...
{
	int i, nbSource;
	nbSource = x->map->get_number_of_sources();
...
void update_source_selection(t_hoamap *x, int sourceIndex, int isSelected){
	int i;
	int nbSources = (int)x->map->get_number_of_sources();
	if (sourceIndex != -1) {
		if (sourceIndex >= 0 && sourceIndex < nbSources) {
			if (isSelected != -1) {
				if (isSelected != 0) isSelected = 1;
				x->map->set_selected(sourceIndex, isSelected);
			} else { // toggle
				x->map->set_selected(sourceIndex, !x->map->get_selected(sourceIndex));
			}
		}
	} else {
		...
		for(i=0; i < nbSources; i++){
			if (i < nbSources) {
				if (isSelected != -1) {
					if (isSelected != 0) isSelected = 1;
					x->map->set_selected(i, isSelected);
				} else { // toggle
					x->map->set_selected(i, !x->map->get_selected(i));
				}
			} else {
				x->map->set_selected(i, 0);
			}
		}
	}
...
void hoamap_output_number_of_sources(t_hoamap *x)
{
	outlet_int(x->f_out, x->map->get_number_of_sources() );
}
...
void hoamap_outone(t_hoamap *x, int i)
{
	...
	hoamap_output_number_of_sources(x);
	
	if (x->map->sourceExist(i)) {
		if (x->f_output_mode == CARTESIAN) {
			atom_setfloat(av+1, (float)x->map->get_x(i) );
			atom_setfloat(av+2, (float)x->map->get_y(i) );
		}
		else if (x->f_output_mode == POLAR) {
			atom_setfloat(av+1, (float)x->map->get_radius(i) );
			atom_setfloat(av+2, (float)x->map->get_angle(i) );
		}
...
void hoamap_add(t_hoamap *x, double xpos, double ypos) {
	...
	if (index != -1) {
		...
		object_error((t_object *)x, "Source can't be added to the scene");
	}
}
...
void hoamap_move(t_hoamap *x, t_symbol *s, short ac, t_atom *av)
{
	...
	if (source >= 0 && coordOk) {
		if (s == gensym("car")) {
			car.x = coord1;
			car.y = coord2;			
			x->map->set_pos(source, car);
			hoamap_outone(x, source);
		}
		else if(s == gensym("pol")){
			if (coord1<0) coord1 *= -1;
			x->map->set_polPos(source, coord1, coord2);
			hoamap_outone(x, source);
		}
		else {
...
void draw_sources(t_hoamap *x,  t_object *view, t_rect *rect)
{
	...
	nbSources = x->map->get_number_of_sources();
	...
	if (g) 
	 {
		...
		for(i = 0; i < nbSources; i++)
		 {
			sourcePos = cartopix(x, x->map->get_pos(i), 0);
			...
			textplace.y = pY(w,x->map->get_y(i) * zoom) + size;
			if (pX(w,x->map->get_x(i)) < w*0.5) {
				textplace.x = sourcePos.x - size - fontsize*0.5;
			}
			if (pY(w,x->map->get_y(i)) < w*0.5) {
		...
		t_jrgba angleStrokeColor = {0,0,0,0.4};
		 ...
		 for(i = nbSources-1; i >= 0  ; i--){
			if (x->map->get_focus(i) && !angleDrawn) {
				...
				t_pt cart = poltocar(x->map->get_radius(i) + (-0.05 / zoom), x->map->get_angle(i));
				...
				sprintf(text,"%.2f °", (float)radtodeg(x->map->get_angle(i)));
...
int which_source_pointed(t_hoamap *x, t_pt pt)
{
	...
	nbSources = x->map->get_number_of_sources();
	for(i = 0; i < nbSources; i++) {
		if (is_in_circle(pt.x, pt.y, pX(x->rect.width,x->map->get_x(i) * x->f_zoomFactor), pY(x->rect.height,x->map->get_y(i) * x->f_zoomFactor), x->f_sourceSize)) {
			sourcePointed = i;
		}
	}
...
void hoamap_mousedown(t_hoamap *x, t_object *patcherview, t_pt pt, long modifiers)
{	
	int nbSources = x->map->get_number_of_sources();
	...
	if (sourceClicked != -1)
	{
	   ...
	   x->f_actualSource = sourceClicked + 1;
		x->map->set_focus(sourceClicked, 1);
	}
	else
	{
...
void hoamap_mousedrag(t_hoamap *x, t_object *patcherview, t_pt pt, long modifiers)
{
	...
	if (x->f_aSourceIsSelected) {
		jmouse_setcursor(patcherview, (t_object *)x, JMOUSE_CURSOR_POINTINGHAND);
		x->map->set_focus(x->f_actualSource-1, 1);
		...
		if(modifiers == 148 || modifiers == 404) // ctrl -> angle.
		{
			...
			diff = polin.a - x->map->get_angle(x->f_actualSource-1);
			for(i = 0; i < nbSources; i++)
			{
				if (x->map->get_selected(i) || i == x->f_actualSource-1) 
				{
					cartemp = poltocar(x->map->get_radius(i), x->map->get_angle(i) + diff);
					cartemp.x = hoaclip(cartemp.x, -10, 10);
					cartemp.y = hoaclip(cartemp.y, -10, 10);
					x->map->set_pos(i, cartemp);
					hoamap_outone(x, i);
		...
		else if(modifiers == 18 || modifiers == 274) // shift -> distance (rayon)
		{
			...
			if (x->map->get_radius(x->f_actualSource-1) != 0.) {
				factor = polin.r / x->map->get_radius(x->f_actualSource-1);
			} else {
				factor = 0.;
			}
			...
			for(i=0; i < nbSources; i++)
			{
				if (x->map->get_selected(i) || i == x->f_actualSource-1) 
				{
					polout.a = x->map->get_angle(i);
					polout.r = x->map->get_radius(i) * factor;
					if (polout.r == 0.) {
						polout.r = 0.00001;
					}
		...
		{
			pixSource = cartopix(x, x->map->get_pos(x->f_actualSource-1), 0);
			...
			for(i=0; i < nbSources; i++)
			{
				if (x->map->get_selected(i) || i == x->f_actualSource-1)
				{
					pixSource = cartopix(x, x->map->get_pos(i), 0);
...
void hoamap_mouseup(t_hoamap *x, t_object *patcherview, t_pt pt, long modifiers)
{
	...
	if (x->f_rect_selection_exist) 
	{
		...
		for(i=0; i < nbSources; i++)
		{
			pix = cartopix(x, x->map->get_pos(i), 0);
