...
{
	t_pxobject					f_ob;			
	AmbisonicSevenDotOne		*f_AmbisonicSevenDotOne;
...
{	
	...
	c = class_new("hoa.sevenDotOne~", (method)HoaSevenDotOne_new, (method)dsp_free, (long)sizeof(t_HoaSevenDotOne), 0L, A_GIMME, 0);
...
void *HoaSevenDotOne_new(t_symbol *s, long argc, t_atom *argv)
{
	...
	if (x = (t_HoaSevenDotOne *)object_alloc((t_class*)HoaSevenDotOne_class)) 
	{
		...
		if(atom_gettype(argv+3) == A_LONG)
			anAngle3	= atom_getlong(argv+3);
		else if(atom_gettype(argv+3) == A_FLOAT)
			anAngle3	= atom_getfloat(argv+3);

		x->f_AmbisonicSevenDotOne	= new AmbisonicSevenDotOne(order, anAngle1, anAngle2, anAngle3, sys_getblksize());
		
		dsp_setup((t_pxobject *)x, x->f_AmbisonicSevenDotOne->getNumberOfInputs());
		for (int i = 0; i < x->f_AmbisonicSevenDotOne->getNumberOfOutputs(); i++) 
			outlet_new(x, "signal");
...
void HoaSevenDotOne_dsp64(t_HoaSevenDotOne *x, t_object *dsp64, short *count, double samplerate, long maxvectorsize, long flags)
{
	x->f_AmbisonicSevenDotOne->setVectorSize(maxvectorsize);
	object_method(dsp64, gensym("dsp_add64"), x, HoaSevenDotOne_perform64, 0, NULL);
}

void HoaSevenDotOne_perform64(t_HoaSevenDotOne *x, t_object *dsp64, double **ins, long numins, double **outs, long numouts, long sampleframes, long flags, void *userparam)
{
	x->f_AmbisonicSevenDotOne->process(ins, outs);
}
...
void HoaSevenDotOne_dsp(t_HoaSevenDotOne *x, t_signal **sp, short *count)
{
	...
	x->f_ninputs = x->f_AmbisonicSevenDotOne->getNumberOfInputs();
	x->f_noutputs = x->f_AmbisonicSevenDotOne->getNumberOfOutputs();
	x->f_AmbisonicSevenDotOne->setVectorSize(sp[0]->s_n);
	pointer_count = x->f_AmbisonicSevenDotOne->getNumberOfInputs() + x->f_AmbisonicSevenDotOne->getNumberOfOutputs() + 2;
...
void HoaSevenDotOne_assist(t_HoaSevenDotOne *x, void *b, long m, long a, char *s)
{
	...
	else
	{
		if (a == 0)
			...
			sprintf(s,"(Signal) Back surround left channel");
		else if (a == 4)
			sprintf(s,"(Signal) Back surround right channel");
		else if (a == 5)
...
void HoaSevenDotOne_infos(t_HoaSevenDotOne *x)
{
	...
	post("Fractional order center channel : %.1f", (float)x->f_AmbisonicSevenDotOne->getFractionalOrderCenter());
	post("Fractional order front channels : %.1f", (float)x->f_AmbisonicSevenDotOne->getFractionalOrderFront());
	post("Fractional order surround channels : %.1f", (float)x->f_AmbisonicSevenDotOne->getFractionalOrderSurround());
	post("Fractional order back surround channels : %.1f", (float)x->f_AmbisonicSevenDotOne->getFractionalOrderBackSurround());
}
