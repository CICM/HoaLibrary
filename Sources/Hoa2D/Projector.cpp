...
namespace Hoa2D
{
    Projector::Projector(unsigned int order, unsigned int numberOfChannels) : Ambisonic(order), Planewaves(numberOfChannels)
    {
        ...
        m_projector_matrix          = new double[m_number_of_channels * m_number_of_harmonics];
        ...
        for(unsigned int i = 0; i < m_number_of_channels; i++)
        {
            ...
            m_projector_matrix[i * m_number_of_harmonics] = 0.5 / (double)(m_order + 1.);
            for(unsigned int j = 1; j < m_number_of_harmonics; j++)
            {
                m_projector_matrix_float[i * m_number_of_harmonics + j] = m_projector_matrix[i * m_number_of_harmonics + j] = m_harmonics_vector[j] / (double)(m_order + 1.);
            }
        }
    }
