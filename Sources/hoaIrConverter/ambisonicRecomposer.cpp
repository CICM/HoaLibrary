...
void ambisonicRecomposer::computeMicMatrix(gsl_matrix* resMatrix, double aFishFactor)
{
	for (int micIndex = 0; micIndex < m_number_of_inputs; micIndex++)
	{
		...
		{
			gsl_matrix_set(resMatrix, micIndex, orderIndex * 2 -1, sin(orderIndex * m_speakers_angles[micIndex]));
			gsl_matrix_set(resMatrix, micIndex, orderIndex * 2   , cos(orderIndex * m_speakers_angles[micIndex]));
		}
	}
}
...
void ambisonicRecomposer::setAngles(double* anAnglesVector)
{
	for(int i = 0; i < m_number_of_inputs; i++)
	{
		m_speakers_angles[i] = (anAnglesVector[i] / 360.) * M_2PI;
		m_speakers_angles[i] = fmod(m_speakers_angles[i] + M_2PI, M_2PI);
	}	
}

void ambisonicRecomposer::setAngles(float* anAnglesVector)
{
	for(int i = 0; i < m_number_of_inputs; i++)
	{
		m_speakers_angles[i] = (anAnglesVector[i] / 360.) * M_2PI;
