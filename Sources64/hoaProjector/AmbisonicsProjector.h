...
#ifndef DEF_AMBISONICSPROJECTOR
#define DEF_AMBISONICSPROJECTOR

#include "../HoaAmbisonics/Ambisonics.h"

class AmbisonicsProjector : public Ambisonics
{
...
public:
	AmbisonicsProjector(long anOrder = 1, long aNumberOfChannels = 0, long aVectorSize = 0);
	~AmbisonicsProjector();
	...
	inline void process(double** aInputs, double** aOutputs)
	{
        for(int i = 0; i < m_vector_size; i++)
		{
            ...
            process(m_vector_double_input, m_vector_double_output);
            for(int j = 0; j < m_number_of_outputs; j++)
            {
                aOutputs[j][i] = m_vector_double_output[j];
            }
        }
	}
