...
#include "AmbisonicsBinaural.hpp"


AmbisonicsBinaural::AmbisonicsBinaural(int aOrder, double aSamplingRate, int aVectorSize, std::string hrtfRootPath, std::string pinnaSize)
 : Ambisonics(aOrder, aVectorSize)

{
	...
	m_decoder = new AmbisonicsDecoder(m_order, m_numberOfVirtualSpeakers);
...
void AmbisonicsBinaural::setVectorSizeAndSamplingRate(int aVectorSize, double aSamplingRate)
{
...
void AmbisonicsBinaural::setPinnaSize(std::string pinnaSize)
{
...
bool AmbisonicsBinaural::formatHrtfFilePath(std::string hrtfRootPath, int aSamplingRate, std::string pinnaSize)
{
...
int	AmbisonicsBinaural::getParameters(std::string aParameter) const
{
	int value = 0;
...
std::string AmbisonicsBinaural::intToString(int aValue)
{
	std::ostringstream oss;
	oss << aValue;
	return oss.str();
}
void AmbisonicsBinaural::computeNbOfVirtualSpeaker()
{
	int possiblesConfigurations[10] = {3, 4, 6, 8, 9, 12, 18, 24, 36, 72};
	m_numberOfVirtualSpeakers = possiblesConfigurations[0];
	for (int i = 0; i < 10; i++)
	{
		if (possiblesConfigurations[i] > getNumberOfHarmonics()) 
		{
...
void AmbisonicsBinaural::loadImpulses()
{
...
void AmbisonicsBinaural::responseInit()
{
...
void AmbisonicsBinaural::matrixResize(int aVectorSize, std::string aMode)
{
...
void AmbisonicsBinaural::setOptimMode(std::string anOptim)
{
...
void AmbisonicsBinaural::computeBasicOptim()
{
	m_optimMode = "basic"; 
...
void AmbisonicsBinaural::computeInPhaseOptim()
{
	m_optimMode = "inPhase"; 
	for (int i = 0; i < m_numberOfHarmonics; i++) 
	{
		if (i == 0) 
			...
			m_optimVector[i] = pow(gsl_sf_fact(m_order), 2) / ( gsl_sf_fact(m_order+abs(getHarmonicIndex(i))) * gsl_sf_fact(m_order-abs(getHarmonicIndex(i))));
	}
}

void AmbisonicsBinaural::computeReOptim()
{
	m_optimMode = "maxRe";
	for (int i = 0; i < m_numberOfHarmonics; i++) 
	{
		if (i == 0) 
			...
			m_optimVector[i] = cos(abs(getHarmonicIndex(i)) * PI / (2*m_order+2));
	}
	
}

void AmbisonicsBinaural::free()
{
