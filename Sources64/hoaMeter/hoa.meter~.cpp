				...
				for(j=0; j<nbLed; j++)
				{
					...
					else if ( drawOverLed )
					{
						...
						if (nLoudSpeak > 1) {
							if ( nLoudSpeak < 24  && (x->f_speakerWidth[i] > 14.5)) {
                                if (x->f_fill_direction == 0) { // inside
                                    jgraphics_arc(g, 0, 0, x->f_rayonInt + ledMargin*2 + x->f_strokeWidth,  deg1+(0.01 * (j+4) * x->f_metersize), deg2-(0.01 * (j+4) * x->f_metersize));
                                } else { // outside
                                    jgraphics_arc(g, 0, 0, x->f_rayonExt - ledMargin*2 - x->f_strokeWidth,  deg1+(0.008 * (nbLed+3-(j)) * x->f_metersize), deg2-(0.008 * (nbLed+3-(j)) * x->f_metersize));
                                }
							}
							else {
                                if (x->f_fill_direction == 0) { // inside
