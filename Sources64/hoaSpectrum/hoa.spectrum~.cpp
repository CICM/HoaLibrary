...
long spectrum_oksize(t_spectrum *x, t_rect *newrect);
...
void draw_angles(t_spectrum *x,  t_object *view, t_rect *rect);
void draw_contribution(t_spectrum *x,  t_object *view, t_rect *rect);
void draw_harmonics(t_spectrum *x,  t_object *view, t_rect *rect);

void spectrum_resize_inputs(t_spectrum *x, long newNumberOfInput);
t_max_err spectrum_setattr_order(t_spectrum *x, t_object *attr, long ac, t_atom *av);
...
int C74_EXPORT main()
{
	...
	class_addmethod(c, (method)spectrum_oksize,		"oksize",		A_CANT, 0);

	CLASS_ATTR_DEFAULT			(c, "patching_rect", 0, "0 0 225 225");

    CLASS_ATTR_LONG             (c, "order", 0, t_spectrum, f_order);
    CLASS_ATTR_ACCESSORS        (c, "order", (method)NULL,(method)spectrum_setattr_order);
    ...
    CLASS_ATTR_LONG             (c, "mode", 0, t_spectrum, f_process_mode);
    ...
    CLASS_ATTR_LONG             (c, "bufsize", 0, t_spectrum, f_bufsize);
    ...
    CLASS_ATTR_ATOM_LONG		(c, "drawcircle", 0, t_spectrum, f_drawCircle);
    ...
    CLASS_ATTR_ATOM_LONG		(c, "drawangles", 0, t_spectrum, f_drawAngles);
    ...
    CLASS_ATTR_ATOM_LONG		(c, "drawcontrib", 0, t_spectrum, f_drawContributions);
	...
	CLASS_ATTR_RGBA				(c, "bgcolor", 0, t_spectrum, f_colorBackground);
    ...
    CLASS_ATTR_RGBA				(c, "bordercolor", 0, t_spectrum, f_bordercolor);
	...
	CLASS_ATTR_RGBA				(c, "txcolor", 0, t_spectrum, f_colorText);
	...
	CLASS_ATTR_RGBA				(c, "phcolor", 0, t_spectrum, f_colorPositif);
	...
	CLASS_ATTR_RGBA				(c, "nhcolor", 0, t_spectrum, f_colorNegatif);
	...
	if (ac && av)
    {
        if (atom_gettype(av) == A_LONG)
        {
            ...
            if (d != x->f_order)
            {
                ...
                x->f_harmonicsValues = new double[x->f_order * 2 + 1];
                x->f_averageHarmo = new double[x->f_order * 2 + 1];
                spectrum_resize_inputs(x, x->f_order * 2 + 1);
...
void spectrum_dsp(t_spectrum *x, t_signal **sp, short *count)
{
	...
	dsp_addv(spectrum_perform, pointer_count, (void **)sigvec);
