...
t_jit_boids2d *jit_boids2d_new(void);
...
void AvoidWalls(t_jit_boids2d *flockPtr, long theBoid, double *wallVel);
...
t_jit_err jit_boids2d_matrix_calc(t_jit_boids2d *flockPtr, void *inputs, void *outputs)
{
	...
	if (flockPtr&&out_matrix) {
		...
		jit_boids2d_calculate_ndim(flockPtr, dimcount, dim, planecount, &out_minfo, out_bp);
	} else {
...
t_jit_boids2d *jit_boids2d_new(void)
{
	t_jit_boids2d *flockPtr;
		
	if (flockPtr=(t_jit_boids2d *)jit_object_alloc(_jit_boids2d_class)) {
		
		flockPtr->flyRectCount		= 4;
		flockPtr->attractPtCount	= 2;
		flockPtr->mode	 			= 0;
		flockPtr->boid 				= NULL;
